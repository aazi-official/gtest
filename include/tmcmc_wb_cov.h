...
void readProposalFiles(RealVector& proposal_mean, RealMatrix& proposal_cov, int ndim, 
            std::string proposalMeanFile, std::string proposalCovFile);
...
public:
  ...
  static double evalProposal(RealVector& x, RealVector& proposal_mean, 
       RealMatrix& proposal_cov_inv, double proposal_cov_det){
    ...
    for(int i = 0; i < ndim ; i++)
      res[i] = x[i] - proposal_mean[i];

    double vMv = vecMatVec(res, proposal_cov_inv, res);

    log_proposal += -0.5*(ndim)*log(2*Pi) - log(proposal_cov_det) - 0.5* vMv;

    return log_proposal;
  }

  static double evalPrior(RealVector& x, double* alphas, double* betas, char* types){
