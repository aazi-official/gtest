...
double tmcmc(int nsamps,
  ...
  */
  ...
  double* proposal_alphas = new double[ndim];
  double* proposal_betas = new double[ndim];
  char* proposal_types = new char[ndim];

  if (pid == 0){
    readPriorParams(proposal_alphas, proposal_betas, proposal_types, ndim, proposalParamsFile, proposalPTypesFile);
  }
  MPI_Bcast(proposal_alphas, ndim, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Bcast(proposal_betas, ndim, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Bcast(proposal_types, ndim, MPI_CHAR, 0, MPI_COMM_WORLD);
  ...
  genPriorSamps(generator, u_distribution, n_distribution,
    spls, nsamps, ndim, proposal_alphas, proposal_betas, proposal_types);
  ...
  do { // Start algorithm
    ...
    for (int isbSteps=0; isbSteps < nSteps; isbSteps++ ) {
      ...
      for (int i = 0; i < nsamps; i++) {
        ...
        lproposal[i] = LogPrior::eval(phi,proposal_alphas,proposal_betas,proposal_types);
      }
